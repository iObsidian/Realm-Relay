...
public class SCry {
	...
	static {
		try {
			X509EncodedKeySpec spec = new X509EncodedKeySpec(DatatypeConverter.parseBase64Binary(SCry.serverPublicKey));
			KeyFactory kf = KeyFactory.getInstance("RSA");
			SCry.key = kf.generatePublic(spec);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public static String createGuestGUID() {
		...
		return SCry.sha1string(timestamp + "" + random + "" + 1).toUpperCase();
	}
	...
	public static String encrypt(String string) {
		...
		try {
			...
			cipher.init(Cipher.ENCRYPT_MODE, SCry.key);
	...
	private static String sha1string(String string) {
		return SCry.hexString(SCry.sha1(SCry.stringToBytes(string)));
	}
